...
namespace Tutorial
{
	...
	public class Game1 : Microsoft.Xna.Framework.Game
	{
		...
		string sWindowTitle = "Tutorial 6 - Position, Orientation, and Pivot Points";
		...
		// The position in 3D space to rotate around
		Vector3 msPivotPoint = new Vector3(0, 50, 0);
...
*/
			...
			mcSpriteBatch.DrawString(mcFont, "Reset Position / Orientation:", new Vector2(5, 375), sPropertyColor);
			mcSpriteBatch.DrawString(mcFont, "Z", new Vector2(265, 375), sControlColor);

			mcSpriteBatch.DrawString(mcFont, "Apply Velocity:", new Vector2(5, 400), sPropertyColor);
			mcSpriteBatch.DrawString(mcFont, "X", new Vector2(150, 400), sControlColor);

			mcSpriteBatch.DrawString(mcFont, "Apply Acceleration:", new Vector2(5, 425), sPropertyColor);
			mcSpriteBatch.DrawString(mcFont, "C", new Vector2(185, 425), sControlColor);

			mcSpriteBatch.DrawString(mcFont, "Rotate Orientation:", new Vector2(5, 450), sPropertyColor);
			mcSpriteBatch.DrawString(mcFont, "V / B / N", new Vector2(180, 450), sControlColor);

			mcSpriteBatch.DrawString(mcFont, "Rotate Position around Pivot Point:", new Vector2(5, 475), sPropertyColor);
			mcSpriteBatch.DrawString(mcFont, "A / S / D", new Vector2(330, 475), sControlColor);

			mcSpriteBatch.DrawString(mcFont, "Rotate Position and Orientation around Pivot Point:", new Vector2(5, 500), sPropertyColor);
			mcSpriteBatch.DrawString(mcFont, "T / Y / U", new Vector2(480, 500), sControlColor);
		...
		public void ProcessInputForParticleSystem(float fElapsedTimeInSeconds)
		{
			...
			//=================================================================
			// Tutorial 6 Specific Code
			//=================================================================

			// Get a handle to the Particle
			DefaultTexturedQuadParticle cParticle = mcMyParticleSystem.ActiveParticles.First.Value;

			if (KeyWasJustPressed(Keys.Z))
			{
				// Reset the Particle's Position and Orientation
			...
			if (KeyWasJustPressed(Keys.X))
			{
				...
				cParticle.Velocity += new Vector3(100, 0, 0);
			}
			else if (KeyWasJustReleased(Keys.X))
			{
			...
			if (KeyIsDown(Keys.C))
			{
				// Make the Particle accelerate
				cParticle.Acceleration = new Vector3(100, 0, 0);
			}
			else
			{
				// Stop the Particle from accelerating (but continue on with its velocity)
				cParticle.Acceleration = Vector3.Zero;
			}
